...
if __name__ == '__main__':
    ...
    def show_tracked_particle(f_no):
        ...
        -------
            figure
        """
        particle_per_frame = tracker.get_particle_per_frame()
        # min = particle_per_frame[f_no]["Minmass"]
        f4 = tp_locate(frames, f_no, tracker.get_diameter(), minmass=particle_per_frame[f_no]["Minmass"],
...
#         """
#         frame = slider.value
#         label.text = f"Minmass: {str(minmass[0])}, Length: {str(length[0])}, Separation: {str(sep[0])},\n" \
#                      f"Maxsize: {str(maxsize[0])}, Topn: {str(topn[0])}, Engine: {str(engine[0])}," \
#                      f"\nMod: {str(mod[0])}"
#         p.image_url(url=[images[frame]], x=-0.76, y=4.11, w=5, h=4.6)
#         pass
